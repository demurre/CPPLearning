...
public interface useCases {
    ...
    List<Article> getArticlesOrderedByTitle() throws DAOException;

    // Print the specified number of articles sorted by the number of pages
    List<Article> getArticlesOrderedByPagesCount() throws DAOException;

    // List articles with more than the specified number of pages
    List<Article> getTheMostArticles(int pagesLimit) throws DAOException;

    // Determine the number of articles in a given subject that have more than a
    // given number of pages
    int countTheMostArticlesInTopic(String topic, int pagesLimit) throws DAOException;
}
